...
class Window(gtk.Window):
	...
	# for reposition method
	self.offset_popup = 0
	self.offset_screen = 4
    ...
    def reposition(self):
	    ...
	    x = window_x + plugin_x + plugin_w/2 - w/2
            ...
            if x < self.offset_screen:
                x = self.offset_screen

            if self.bar.cfg['position'] == "top":
                y = window_y + window_h + self.offset_popup
            else:
                y = window_y - h - self.offset_popup

            while x >= self.offset_screen and x + w + self.offset_screen > gtk.gdk.screen_width():
                x = x - 1
            ...
            if self.bar.cfg['position'] == "left":
                x = window_x + window_w + self.offset_popup
            else:
                x = window_x - w - self.offset_popup
